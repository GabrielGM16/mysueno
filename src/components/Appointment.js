...
function Appointment() {
  ...
  const handleSpecialtyChange = async (e) => {
    ...
    const token = localStorage.getItem('token');
    try {
      const response = await axios.get(`http://ec2-3-235-102-184.compute-1.amazonaws.com/api/specialists/${e.target.value}`, {
        headers: {
          Authorization: `Bearer ${token}`
        }
      });
  ...
  const handleSubmit = async (e) => {
    ...
    const token = localStorage.getItem('token');
    try {
      await axios.post('http://ec2-3-235-102-184.compute-1.amazonaws.com/api/appointments', {
        userId,
