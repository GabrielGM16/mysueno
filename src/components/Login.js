...
const Login = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (step === 1) {
        const response = await axios.post('http://ec2-3-235-102-184.compute-1.amazonaws.com/api/auth/login', { email: formData.email, password: formData.password });
        ...
        const response = await axios.post('http://ec2-3-235-102-184.compute-1.amazonaws.com/api/auth/verify-code', { email, code: formData.code });
